...
export class InitiativeTrackerGroupAdd extends BaseComponent {
	...
	_render_$getStgIpts ({rdState}) {
		...
		// const $initiative_strat = ComponentUiUtil.$getSelEnum(this, "initiative_strat", {values: {0, 1, 2}, html=})
		
		const $cb = $(`<input type="radio" id="html" name="fav_language" value="HTML">
						<label for="html">Individual</label><br>
						<input type="radio" id="css" name="fav_language" value="CSS">
						<label for="css">Group</label><br>
						<input type="radio" id="javascript" name="fav_language" value="JavaScript">
						<label for="javascript">Flat bonus:</label>`);

		const $initiative_bonus = ComponentUiUtil.$getIptStr(this, "initiative_bonus", {html: `<input class="form-control">`})
			.on("keydown", evt => {
				if (evt.key !== "Enter") return;
				$initiative_bonus.trigger("change");
				this._doSubmit({rdState});
			});
		...
		return $$`
			<div class="ve-flex-v-center mb-2">
				...
				<div class="small-caps ve-col-4 px-1">Initiative strategy</div>
				<div class="small-caps ve-col-4 px-1">Initiative bonus</div>
			...
			<div class="ve-flex-v-center mb-3">
				...
				<div class="ve-col-4 px-1">${$cb}</div>
				<div class="ve-col-4 px-1">${$initiative_bonus}</div>
			</div>
		`;
	}
